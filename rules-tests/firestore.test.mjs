...
await check(
  'authed user cannot read non-discoverable other profile',
  'deny',
  getDoc(doc(aliceDb(), 'profiles', CAROL)),
);
await check(
  'owner can read own non-discoverable profile',
  'allow',
  getDoc(doc(carolDb(), 'profiles', CAROL)),
);
await check('unauthenticated cannot read profile', 'deny', getDoc(doc(anonDb(), 'profiles', BOB)));
...
await check(
  'user can create own outgoing connect swipe',
  'allow',
...
await check(
  'user cannot create swipe with fromUid != auth uid',
  'deny',
...
await check(
  'swipe id must equal fromUid_toUid',
  'deny',
...
await check(
  'swipe action must be connect/skip',
  'deny',
...
await check(
  'target can read incoming swipe (mutual-match check)',
  'allow',
  getDoc(doc(bobDb(), 'swipes', `${ALICE}_${BOB}`)),
);
await check(
  'unrelated user cannot read a swipe',
  'deny',
...
await check(
  'user can get a non-existent incoming swipe (Connect mutual-match check)',
  'allow',
...
await check('member can read match', 'allow', getDoc(doc(aliceDb(), 'matches', M1)));
await check('non-member cannot read match', 'deny', getDoc(doc(carolDb(), 'matches', M1)));
...
await check(
  'user can get an expected match before it exists',
  'allow',
  getDoc(doc(aliceDb(), 'matches', `${ALICE}_${CAROL}`)),
);
await check(
  'member can update lastMessage/lastSentAt',
  'allow',
...
await check(
  'member cannot change memberUids',
  'deny',
  updateDoc(doc(aliceDb(), 'matches', M1), { memberUids: [ALICE, CAROL] }),
);
await check(
  'authenticated user cannot create a match directly from the client',
  'deny',
  setDoc(doc(aliceDb(), 'matches', `${ALICE}_${CAROL}`), {
    matchId: `${ALICE}_${CAROL}`,
...
await check('client cannot delete a match', 'deny', deleteDoc(doc(aliceDb(), 'matches', M1)));
...
await check(
  'member can create message with fromUid == auth.uid',
  'allow',
...
await check(
  'member cannot create message with fromUid != auth.uid',
  'deny',
  addDoc(collection(aliceDb(), 'matches', M1, 'messages'), {
    fromUid: BOB,
    text: 'spoof',
...
);
{
  ...
  batch.set(
    doc(db, 'matches', M1),
    {
      lastMessage: 'batched hello',
      lastSentAt: new Date(),
    },
    { merge: true },
  );
  await check('member can batch message and match preview', 'allow', batch.commit());
}
{
  const db = aliceDb();
  const batch = writeBatch(db);
  ...
  batch.set(
    doc(db, 'matches', M1, 'memberStates', BOB),
    {
  ...
  await check(
    'member cannot batch message, match preview, and recipient unread',
    'deny',
    batch.commit(),
  );
}
...
await check(
  'member can read own memberState',
  'allow',
  getDoc(doc(bobDb(), 'matches', M1, 'memberStates', BOB)),
);
await check(
  'member cannot read another memberState',
  'deny',
  getDoc(doc(aliceDb(), 'matches', M1, 'memberStates', BOB)),
);
await check(
  'non-member cannot read memberState',
  'deny',
  getDoc(doc(carolDb(), 'matches', M1, 'memberStates', BOB)),
);
await check(
  'member can mark own match read',
  'allow',
  setDoc(
    doc(bobDb(), 'matches', M1, 'memberStates', BOB),
    {
      unreadCount: 0,
      lastReadAt: new Date(),
      updatedAt: new Date(),
...
await check(
  'member can create own memberState',
  'allow',
  setDoc(
    doc(aliceDb(), 'matches', M1, 'memberStates', ALICE),
    {
      unreadCount: 0,
      muted: false,
      updatedAt: new Date(),
...
await check(
  'sender match member cannot write recipient unread',
  'deny',
  setDoc(
    doc(aliceDb(), 'matches', M1, 'memberStates', BOB),
    {
      unreadCount: 2,
      updatedAt: new Date(),
    },
...
await check(
  'sender cannot update recipient lastReadAt',
  'deny',
...
await check(
  'non-member cannot write memberState',
  'deny',
  setDoc(
    doc(carolDb(), 'matches', M1, 'memberStates', BOB),
    {
      unreadCount: 3,
      updatedAt: new Date(),
    },
...
await check(
  'user can read own entitlement',
  'allow',
  getDoc(doc(aliceDb(), 'entitlements', ALICE)),
);
await check(
  'user cannot read another entitlement',
  'deny',
  getDoc(doc(bobDb(), 'entitlements', ALICE)),
);
await check(
  'client cannot write entitlement',
  'deny',
...
await check(
  'cannot create block with blockerUid != auth uid',
  'deny',
...
await check('blocker can read own block', 'allow', getDoc(doc(aliceDb(), 'blocks', BLOCK_ID)));
await check('blocked user can read the block', 'allow', getDoc(doc(bobDb(), 'blocks', BLOCK_ID)));
await check('unrelated user cannot read block', 'deny', getDoc(doc(carolDb(), 'blocks', BLOCK_ID)));
...
await check('normal user cannot list reports', 'deny', getDocs(collection(aliceDb(), 'reports')));
...
await check(
  'normal user cannot read report review',
  'deny',
  getDoc(doc(aliceDb(), 'reportReviews', 'seed1')),
);
await check(
  'normal user cannot list report reviews',
  'deny',
  getDocs(collection(aliceDb(), 'reportReviews')),
);
await check(
  'normal user cannot write report review',
  'deny',
...
await check(
  'normal user cannot read moderation review',
  'deny',
  getDoc(doc(aliceDb(), 'moderationReviews', 'review1')),
);
await check(
  'normal user cannot write moderation review',
  'deny',
...
await check(
  'reported user cannot read own moderation status',
  'deny',
  getDoc(doc(bobDb(), 'userModeration', BOB)),
);
await check(
  'unrelated user cannot read moderation status',
  'deny',
  getDoc(doc(carolDb(), 'userModeration', BOB)),
);
await check(
  'normal user cannot write user moderation status',
  'deny',
...
await seed(async (db) => {
  await setDoc(doc(db, 'accountDeletionRequests', BOB), {
    ...
    retentionPolicyVersion: 'draft-1',
...
await check(
  'user can create own account deletion request',
  'allow',
...
await check(
  'user can read own account deletion request',
  'allow',
  getDoc(doc(bobDb(), 'accountDeletionRequests', BOB)),
);
await check(
  "user cannot read another user's account deletion request",
  'deny',
  getDoc(doc(aliceDb(), 'accountDeletionRequests', BOB)),
);
await check(
  'user cannot list all account deletion requests',
  'deny',
  getDocs(collection(aliceDb(), 'accountDeletionRequests')),
);
await check(
  'user cannot create account deletion request for another uid',
  'deny',
...
await check(
  'account deletion request uid must match doc id',
  'deny',
  setDoc(doc(aliceDb(), 'accountDeletionRequests', ALICE), {
    uid: BOB,
    requestId: ALICE,
...
await check(
  'account deletion request status is client-fixed to requested',
  'deny',
  setDoc(doc(aliceDb(), 'accountDeletionRequests', ALICE), {
    uid: ALICE,
    requestId: ALICE,
...
await check(
  'owner can cancel scheduled deletion request',
  'allow',
  updateDoc(doc(bobDb(), 'accountDeletionRequests', BOB), {
    status: 'cancelled',
...
await check(
  'audit collection is client denied',
  'deny',
  getDoc(doc(bobDb(), 'accountDeletionAudit', 'audit')),
);
...
const pushToken = (uid, deviceId, extra = {}) => ({
  uid,
  token: 'ExponentPushToken[not-a-real-token]',
  platform: 'ios',
  deviceId,
  enabled: true,
  createdAt: new Date(),
...
await check(
  'owner can register own push token',
  'allow',
  setDoc(
    doc(aliceDb(), 'users', ALICE, 'pushTokens', 'alice-device'),
    pushToken(ALICE, 'alice-device'),
  ),
);
await check(
  'other user cannot register a token for owner',
  'deny',
  setDoc(
    doc(aliceDb(), 'users', BOB, 'pushTokens', 'other-device'),
    pushToken(BOB, 'other-device'),
  ),
);
await check(
  'other user cannot update a token',
  'deny',
  updateDoc(doc(aliceDb(), 'users', BOB, 'pushTokens', 'bob-device'), { enabled: false }),
);
await check(
  'owner cannot swap token uid',
  'deny',
  updateDoc(doc(bobDb(), 'users', BOB, 'pushTokens', 'bob-device'), { uid: ALICE }),
);
await check(
  'owner can invalidate current token fields',
  'allow',
