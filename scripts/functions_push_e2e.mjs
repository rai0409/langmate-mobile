...
const base = (id, retryCount = 0) => ({
  ...
  matchId: 'functions-push-match',
  messageId: id,
  senderUid: 'functions-push-sender',
  recipientUid: recipient,
  idempotencyKey: `message_received:functions-push-match:${id}:${recipient}`,
  status: 'pending',
  retryCount,
...
await db.doc(`users/${recipient}/pushTokens/device-a`).set({
  uid: recipient,
  token: 'ExponentPushToken[e2e-a]',
  platform: 'ios',
  deviceId: 'device-a',
  enabled: true,
  createdAt: new Date(),
...
await db.doc(`users/${recipient}/pushTokens/device-b`).set({
  uid: recipient,
  token: 'ExponentPushToken[e2e-b]',
  platform: 'android',
  deviceId: 'device-b',
  enabled: false,
  createdAt: new Date(),
...
if (sent.ticketIds.length !== 1 || sent.receiptStatus !== 'pending')
  throw new Error('expected one enabled token ticket');
await success.set({ check: 'rewrite' }, { merge: true });
await new Promise((r) => setTimeout(r, 800));
if ((await success.get()).data().ticketIds.length !== 1)
  throw new Error('rewrite duplicated delivery');
await processReceipts(db, new FakePushProvider('success'));
if ((await success.get()).data().receiptStatus !== 'confirmed')
...
if ((await retry.get()).data().status !== 'retryable_failed')
  throw new Error('retryable failure did not schedule retry');
...
if ((await exhausted.get()).data().status !== 'permanently_failed')
  throw new Error('retry limit did not terminate');
...
if ((await db.doc(`users/${recipient}/pushTokens/device-a`).get()).data().enabled !== false)
  throw new Error('invalid token was not disabled');
...
if ((await noTokens.get()).data().status !== 'skipped')
  throw new Error('missing tokens did not skip');
...
if ((await db.doc(`users/${recipient}/pushTokens/device-a`).get()).data().enabled !== false)
  throw new Error('receipt invalid token was not disabled');
