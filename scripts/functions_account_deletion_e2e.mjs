...
await db.doc(`blocks/${uid}_${other}`).set({});
await db.doc(`blocks/${other}_third`).set({});
const steps = [];
const prefixes = [];
await Promise.all([
  processDeletion(uid, db, {
...
for (const d of audit.docs) {
  ...
  assert.ok(!x.includes('never-audit@example.test') && !x.includes('secret-token'));
}
