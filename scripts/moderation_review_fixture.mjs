...
const fixturePath = join(__dirname, 'fixtures', 'moderation_reports_fixture.json');

const VALID_REVIEW_STATUSES = new Set(['open', 'reviewing', 'actioned', 'dismissed']);
...
function buildReview(report, defaults) {
  ...
  const action = requireOneOf(defaults.action ?? 'none', VALID_ACTIONS, 'action', 'reviewDefaults');
...
const reviews = reports.map((report) => buildReview(report, fixture.reviewDefaults ?? {}));
