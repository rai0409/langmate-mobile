        ...
        (async () => {
          const next = await Promise.all(
            matches.map(async (match) => {
              const partnerUid = match.memberUids.find((uid) => uid !== currentUser.uid) ?? '';
  ...
  useEffect(() => {
    ...
    const unsubscribe = listenBlocksForUser(
      ...
      (error) => {
        ...
        setWarning('Could not load your block list — matches with blocked users may still appear.');
      },
    );
    return unsubscribe;
  }, [currentUser]);
    ...
    <FlatList
        ...
        <>
          <View style={styles.infoCard}>
            ...
            <Text style={styles.infoText}>
              Badges show unread messages inside the app. Push notifications are coming later.
            </Text>
          </View>
          {warning ? <Text style={styles.warning}>{warning}</Text> : null}
        </>
      ...
      renderItem={({ item }) => (
        <View style={styles.row}>
          <Pressable
            style={({ pressed }) => [styles.profileTarget, pressed && styles.pressed]}
            onPress={() => openPartnerProfile(item)}
          >
            <View style={styles.avatarWrap}>
              <ProfileAvatar profile={item.partnerProfile} name={item.partnerName} size={44} />
            </View>
