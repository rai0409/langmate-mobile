...
import { filterAndRankCandidates, type RankedCandidate } from '../services/matchingService';
...
export function DiscoverScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (hasFirebaseConfig()) {
        ...
        if (currentUser) {
          try {
            blockSets = toBlockSets(currentUser.uid, await listBlocksForUser(currentUser.uid));
        ...
        const others = real.filter(
          (p) => p.uid !== currentUser?.uid && !isUidBlocked(blockSets, p.uid),
        );
        if (others.length > 0) {
  ...
  const handleConnect = async (candidate: RankedCandidate) => {
    ...
    try {
      ...
      const match = await createMatchIfMutualConnect(currentUser.uid, candidate.profile.uid);
      if (match) {
        notify(
          "It's a match!",
  ...
  return (
    <View style={styles.screen}>
      {blockWarning ? <Text style={styles.blockWarning}>{blockWarning}</Text> : null}
      {visible.length === 0 ? (
        <EmptyState
          ...
          ListHeaderComponent={
            isPreview ? (
              <Text style={styles.previewNote}>
                Showing preview data — set up Firebase (or wait for real users) to discover actual
                partners.
              </Text>
            ) : null
          }
