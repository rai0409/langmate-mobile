...
assert.deepEqual(classifyProviderCode('ServiceUnavailable'), {
  retryable: true,
  permanent: false,
  invalidToken: false,
});
assert.deepEqual(classifyProviderCode('DeviceNotRegistered'), {
  retryable: false,
  permanent: true,
  invalidToken: true,
});
...
assert.equal(
  normalizeReceipt('ticket', { status: 'error', details: { error: 'DeviceNotRegistered' } })
    .invalidToken,
  true,
);
...
assert.equal(
  canClaimOutbox(
    { status: 'pending', nextAttemptAt: Timestamp.fromMillis(now.toMillis() - 1) },
    now,
  ),
  true,
);
assert.equal(
  canClaimOutbox(
    { status: 'processing', leaseExpiresAt: Timestamp.fromMillis(now.toMillis() + 1) },
    now,
  ),
  false,
);
