...
export function canClaimOutbox(outbox: Outbox, now = nowTimestamp()): boolean {
  ...
  return (
    ((outbox.status === 'pending' || outbox.status === OUTBOX_STATUS_RETRYABLE_FAILED) && due) ||
...
function safeOutbox(
  ...
  Pick<Outbox, 'type' | 'matchId' | 'messageId' | 'senderUid' | 'recipientUid' | 'idempotencyKey'>
> {
  return [
    outbox.type,
    outbox.matchId,
...
async function terminal(
  db: Firestore,
  id: string,
  status: typeof OUTBOX_STATUS_SKIPPED | typeof OUTBOX_STATUS_PERMANENTLY_FAILED,
  reason: string,
): Promise<void> {
  await db
    ...
    .update({
      ...
      failedAt: status === OUTBOX_STATUS_PERMANENTLY_FAILED ? FieldValue.serverTimestamp() : null,
      leaseExpiresAt: null,
    });
}

export async function processOutboxById(
  db: Firestore,
  outboxId: string,
  provider: PushProvider | null = getPushProvider(),
): Promise<void> {
  const started = Date.now();
  const outbox = await claim(db, outboxId);
  if (!outbox) return;
  const attempt = (outbox.retryCount ?? 0) + 1;
  if (!safeOutbox(outbox))
    return terminal(db, outboxId, OUTBOX_STATUS_PERMANENTLY_FAILED, 'invalid_outbox');
  if (!provider)
    return terminal(db, outboxId, OUTBOX_STATUS_PERMANENTLY_FAILED, 'provider_not_configured');
  const tokenSnapshots = await db
    .collection(`users/${outbox.recipientUid}/pushTokens`)
    .where('enabled', '==', true)
    .get();
  ...
  if (targets.length === 0)
    return terminal(db, outboxId, OUTBOX_STATUS_SKIPPED, 'no_enabled_tokens');
  const payload = makeMessagePayload({
    matchId: outbox.matchId,
    senderId: outbox.senderUid,
    messageId: outbox.messageId,
    notificationType: outbox.type,
  ...
  try {
    ...
    await Promise.all(
      invalid.map((result) =>
        db.doc(`users/${outbox.recipientUid}/pushTokens/${result.target.tokenId}`).update({
          enabled: false,
          invalidatedAt: FieldValue.serverTimestamp(),
          invalidReason: result.code ?? 'invalid_token',
        }),
      ),
    );
    ...
    if (tickets.length > 0) {
      await ref.update({
        ...
        ticketIds: tickets,
        ticketTokenIds: Object.fromEntries(
          results.flatMap((r) => (r.ticketId ? [[r.ticketId, r.target.tokenId]] : [])),
        ),
        receiptStatus: 'pending',
        receiptCheckCount: 0,
        sentAt: FieldValue.serverTimestamp(),
        providerResponse: results.map((r) => ({
          ok: r.ok,
          ...(r.code ? { code: r.code } : {}),
          ...(r.ticketId ? { ticketId: r.ticketId } : {}),
          tokenHash: tokenFingerprint(r.target.token),
        })),
        leaseExpiresAt: null,
      });
    } else if (retryable && attempt < MAX_PUSH_RETRIES) {
      await ref.update({
        status: OUTBOX_STATUS_RETRYABLE_FAILED,
        retryCount: attempt,
        nextAttemptAt: Timestamp.fromMillis(Date.now() + retryDelayMs(attempt)),
        providerResponse: results.map((r) => ({ ok: r.ok, ...(r.code ? { code: r.code } : {}) })),
        leaseExpiresAt: null,
      });
    } else {
      await terminal(
        ...
        outboxId,
        OUTBOX_STATUS_PERMANENTLY_FAILED,
        allPermanent ? 'provider_permanent_failure' : 'retry_limit_exceeded',
      );
    }
    logger.info('push_outbox_processed', {
      outboxId,
      notificationType: outbox.type,
      attempt,
      provider: provider.name,
      normalizedResult: tickets.length
        ? 'ticketed'
        : retryable
          ? 'retryable_failed'
          : 'permanently_failed',
      ...
      terminalStatus: tickets.length
        ? null
        : retryable && attempt < MAX_PUSH_RETRIES
          ? null
          : OUTBOX_STATUS_PERMANENTLY_FAILED,
    });
  } catch {
    ...
    if (attempt >= MAX_PUSH_RETRIES)
      await terminal(
        ...
        outboxId,
        OUTBOX_STATUS_PERMANENTLY_FAILED,
        'provider_transport_retry_limit',
      );
    else
      await ref.update({
        status: OUTBOX_STATUS_RETRYABLE_FAILED,
    ...
    logger.warn('push_outbox_transport_failure', {
      outboxId,
      notificationType: outbox.type,
      attempt,
      provider: provider.name,
...
export async function processReceipts(
  ...
  const snapshot = await db
    .collection('notificationOutbox')
    .where('status', '==', OUTBOX_STATUS_SENT)
  ...
  for (const document of snapshot.docs) {
    ...
    if (checks > MAX_RECEIPT_CHECKS) {
      await document.ref.update({ receiptStatus: 'exhausted', receiptCheckCount: checks });
      continue;
    }
    try {
      ...
      if (invalid.length > 0 && outbox.recipientUid) {
        ...
        await Promise.all(
          invalid.flatMap((receipt) => {
            const tokenId = ticketTokenIds[receipt.ticketId];
            return tokenId
              ? [
                  db.doc(`users/${outbox.recipientUid}/pushTokens/${tokenId}`).update({
                    enabled: false,
                    invalidatedAt: FieldValue.serverTimestamp(),
      ...
      if (receipts.some((r) => r.retryable))
        await document.ref.update({
          receiptStatus: 'retryable_failed',
          receiptCheckCount: checks,
          nextReceiptAttemptAt: Timestamp.fromMillis(Date.now() + retryDelayMs(checks)),
        });
      else if (receipts.some((r) => r.permanent))
        await document.ref.update({
          receiptStatus: 'permanently_failed',
          receiptCheckCount: checks,
      ...
      await document.ref.update({
        receiptCheckCount: checks,
        receiptStatus: checks >= MAX_RECEIPT_CHECKS ? 'exhausted' : 'pending',
      });
    }
  }
}

export const deliverNotificationOutbox = onDocumentCreated(
...
export const retryNotificationOutbox = onSchedule('every 5 minutes', async () => {
  ...
  const retryable = await db
    .collection('notificationOutbox')
    .where('status', 'in', ['pending', OUTBOX_STATUS_RETRYABLE_FAILED])
    .limit(100)
    .get();
